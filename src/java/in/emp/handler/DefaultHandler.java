 ...
 * @author rushi
 */
public class DefaultHandler implements GenericFormHandler {
    ...
    public String execute(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if (!ApplicationUtils.isBlank(uiActionName)) {
                ...
                } else if (uiActionName.equals(ApplicationConstants.UIACTION_GET_ALL_NOTIFICATIONS)) {
                    sReturnPage = getAllNotifications(request);
                } else {
    ...
    public String getAllNotifications(HttpServletRequest request) throws Exception {
        String sReturnPage = ApplicationConstants.UIACTION_GET_ALL_NOTIFICATIONS;
        NotificationsPrezData notificationsPrezDataObj = new NotificationsPrezData();
        NotificationsBean notificationsBeanObj = new NotificationsBean();
        NotificationsDelegate notificationsMgrObj = new NotificationsManager();
        HttpSession session = request.getSession();
        try {
            logger.log(Level.INFO, "PersonalInfoHandler :: getAllNotifications() :: method called :: ");
            ...
            notificationsPrezDataObj.setNotificationBeanObj(notificationsBeanObj);
            notificationsPrezDataObj = notificationsMgrObj.getAllNotificationsList(notificationsPrezDataObj);

            session.setAttribute(ApplicationConstants.NOTIFICATIONS_SESSION_DATA, notificationsPrezDataObj);

        } catch (Exception ex) {
            logger.log(Level.ERROR, "PersonalInfoHandler :: getAllNotifications() :: Exception :: " + ex);
        }
        return sReturnPage;
    }
}
